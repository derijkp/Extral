...
/*------------------------------------------------------------------*/
	...
	if (regpattern==NULL) {
		...
		L_pos->regexpPtr = (Tcl_RegExp *)Tcl_RegExpCompile(interp, regpattern);
...
/*------------------------------------------------------------------*/
	...
	Tcl_RegExp *regexpPtr=L_pos->regexpPtr;
	char *tline;
	char *result=NULL;
	int match = 0;
	int begin,end,len;
	...
	if (regexpPtr==NULL) {
		...
		if (!match) {
			...
			Tcl_DString element;
			char savedChar, *first, *last, *nfirst, *nlast;
			Tcl_RegExpRange((Tcl_RegExp)regexpPtr,1,&first,&last);
			Tcl_RegExpRange((Tcl_RegExp)regexpPtr,2,&nfirst,&nlast);
			if (first==NULL) {
				Tcl_AppendElement(interp,result);
			} else if (nfirst==NULL) {
				savedChar = *last;
				...
				*last = savedChar;
				...
				while (first!=NULL) {
					...
					*last = savedChar;
					i++;
					Tcl_RegExpRange((Tcl_RegExp)regexpPtr,i,&first,&last);
				}
